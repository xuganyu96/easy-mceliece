...
static int mov_columns(uint64_t mat[][(nBlocks_I + 1) * 4], int16_t *pi,
                       uint64_t *pivots) {
...
int pk_gen(unsigned char *pk, const unsigned char *irr, uint32_t *perm,
  ...
  // Gaussian elimination + column swaps to obtain L, U, and P such that LP M =
  // U L and U are stored in the space of M P is stored in ind
  ...
  for (i = 1; i < SYS_T; i++) {
    for (b = 1; b < par_width; b++)
      vec256_mul(prod[nBlocks_I + b], prod[nBlocks_I + b],
                 consts[nBlocks_I + b]);
