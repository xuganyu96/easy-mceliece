...
static inline void interleave(vec256 *in, int idx0, int idx1, vec256 *mask,
                              int b) {
  int s = 1 << b;
...
static inline void get_coefs(gf *out, vec256 *in) {
  ...
  for (i = 0; i < 16; i++)
    for (k = 0; k < 4; k++) {
      out[(4 * 0 + k) * 16 + i] =
          (vec256_extract(buf[i], 0) >> (k * 16)) & GFMASK;
      out[(4 * 1 + k) * 16 + i] =
          (vec256_extract(buf[i], 1) >> (k * 16)) & GFMASK;
      out[(4 * 2 + k) * 16 + i] =
          (vec256_extract(buf[i], 2) >> (k * 16)) & GFMASK;
      out[(4 * 3 + k) * 16 + i] =
          (vec256_extract(buf[i], 3) >> (k * 16)) & GFMASK;
    }
}
