...
static inline void minmax_rows(uint16_t *x, vec256 (*mat)[par_width], int i0,
                               int i1) {
  int i;
  uint16_t m;
  vec256 mm;
...
static void merge_rows(int n, int bound, uint16_t *x, vec256 (*mat)[par_width],
                       int off, int step) {
  int i;
...
static void sort_rows(int n, int bound, uint16_t *x, vec256 (*mat)[par_width],
                      int off) {
  if (n <= 1)
    return;
...
static void to_bitslicing_2x(vec256 out0[][GFBITS], vec256 out1[][GFBITS],
                             const uint64_t *in) {
  int i, j, k, r;
  uint64_t u[2][4];
...
static int mov_columns(uint64_t mat[][nBlocks_I * 4], int16_t *pi,
                       uint64_t *pivots) {
