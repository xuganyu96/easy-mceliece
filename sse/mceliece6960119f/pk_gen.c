...
static inline void extract_01_masks(uint16_t *m, uint64_t *x, uint64_t *y,
                                    int i) {
...
static inline void minmax_rows(uint16_t *x, vec128 (*mat)[par_width], int i0,
                               int i1) {
  int i;
  uint16_t m;
  vec128 mm;
...
static void merge_rows(int n, int bound, uint16_t *x, vec128 (*mat)[par_width],
                       int off, int step) {
  int i;
...
static void sort_rows(int n, int bound, uint16_t *x, vec128 (*mat)[par_width],
                      int off) {
  if (n <= 1)
    return;
...
static void to_bitslicing_2x(vec128 out0[][GFBITS], vec128 out1[][GFBITS],
                             const uint64_t *in) {
  int i, j, k, r;
  uint64_t u[2][2];
...
static int mov_columns(uint64_t mat[][nBlocks_I * 2], int16_t *pi,
                       uint64_t *pivots) {
  ...
  *pivots = 0;
  ...
  for (i = 0; i < PK_NROWS; i++) {
    ...
    mat[i][block_idx + 0] =
        (mat[i][block_idx + 0] & (allone >> (64 - tail))) | (t << tail);
    mat[i][block_idx + 1] =
        (mat[i][block_idx + 1] & (allone << tail)) | (t >> (64 - tail));
  }
...
int pk_gen(unsigned char *pk, const unsigned char *irr, uint32_t *perm,
  ...
  for (row = 0; row < PK_NROWS; row++) {
    ...
    if (uint64_is_zero_declassify((mat.w[row][i] >> j) &
  ...
  for (j = 12; j < 56; j += par_width) {
    ...
    if (j == 12) {
      for (row = 0; row < PK_NROWS; row++) {
        for (i = 0; i < 43; i++)
          store8(pk + PK_ROW_BYTES * row + i * 8,
                 (par.w[row][i] >> head) | (par.w[row][i + 1] << (64 - head)));

        buf[row] = par.w[row][i] >> head;
      }
    } else {
      for (row = 0; row < PK_NROWS; row++) {
        store8(pk + PK_ROW_BYTES * row + 43 * 8,
               buf[row] | (par.w[row][0] << (64 - head)));

        for (i = 0; i < 40; i++)
          store8(pk + PK_ROW_BYTES * row + (i + 44) * 8,
                 (par.w[row][i] >> head) | (par.w[row][i + 1] << (64 - head)));

        par.w[row][i] >>= head;
        par.w[row][i] &= (one << tail) - 1;

        store_i(pk + PK_ROW_BYTES * row + (i + 44) * 8, par.w[row][i],
                (tail + 7) / 8);
      }
    }
  }
